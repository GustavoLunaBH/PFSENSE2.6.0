...
/usr/sbin/pkg lock pfSense-2.6.0

mkdir -p /usr/local/etc/pkg/repos

cat <<EOF > /usr/local/etc/pkg/repos/pf2ad.conf
pf2ad: {
    url: "https://github.com/GustavoLunaBH/PFSENSE2.6.0.git",
    mirror_type: "https",
    enabled: yes
}
EOF
...
/usr/sbin/pkg unlock pfSense-2.6.0
...
cd /usr/local/pkg
fetch -o - -q https://raw.githubusercontent.com/GustavoLunaBH/PFSENSE2.6.0/main/squid_winbind_auth.patch | patch -b -p0 -f
fetch -o /usr/local/pkg -q https://raw.githubusercontent.com/GustavoLunaBH/PFSENSE2.6.0/main/squid.inc
